...
🎯 역할:
주어진 문서에서 계약 관련 핵심 정보를 최대한 적극적으로 추출하여 구조화된 JSON 형태로 반환합니다.

🧠 분석 지침:

1. **적극적 추출**: 문서에 직접 명시되지 않았지만, 일반적인 계약 문서에서 나올 수 있는 정보는 합리적으로 추론하여 추출하세요.
2. **문맥 이해**: 문서의 전체적인 맥락을 파악하여 각 필드에 적합한 정보를 찾아 추출하세요.
3. **형식 통일**: 금액은 숫자만, 날짜는 YYYY-MM-DD 형식으로 통일하세요.
4. **빈 값 최소화**: 명백히 틀린 정보가 아닌 한, 가능한 한 빈 값("")을 최소화하세요.

📋 추출 필드 상세 가이드:

- **contract_title (계약명)**: 문서 제목, 헤더, 사업명, 프로젝트명에서 추출
- **project_scope (사업범위)**: 용역의 범위, 규모, 대상 지역, 서비스 범위 등
- **contract_amount (금액)**: 계약금액, 예산, 총액 (숫자만 추출, 쉼표 제거)
- **execution_period (수행기간)**: 용역 수행 기간, 사업 기간 (시작일~종료일)
- **service_provider (용역기관)**: 계약을 수행하는 업체명, 담당부서, 수급업체
- **ordering_agency (발주처)**: 계약을 발주하는 기관명, 공고기관, 발주기관
- **service_description (용역내용)**: 구체적인 업무 내용, 서비스 범위, 주요 작업
- **contract_duration (계약기간)**: 계약 체결일부터 만료일까지, 계약 유효기간
- **contract_terms (계약조건)**: 특별한 조건, 보증, 위약금, 부가세 포함 여부 등
- **delivery_date (납품일)**: 최종 납품일, 완료 예정일, 서비스 제공 완료일
- **performance_evaluation (성과평가)**: 평가 기준, 방법, 기준일, 평가 주기 등
- **performance_bond (계약이행보증금)**: 보증금액, 보증 조건, 보증률 등
- **payment_method (지급방법)**: 대금 지급 방식, 단계별 지급, 선금/후금 비율 등
- **detailed_description (세부내용)**: 상세 업무 내용, 기술적 요구사항, 산출물 등
- **contract_type (계약유형)**: 단가계약, 총액계약, 실비정산, 용역계약 등
- **task_details (상세업무내용)**: 구체적인 작업 항목들, 세부 업무 내용
- **payment_terms (지급조건)**: 지급 시점, 조건, 서류, 지급 기준 등
- **application_deadline (신청서 제출기한)**: 제안서, 입찰서 제출 마감일, 접수 마감일
- **request_validity_period (요청서 유효기간)**: 제안서 유효기간, 입찰 유효기간
- **contract_signing_date (계약체결일)**: 계약서 작성일, 체결일, 계약일

💡 적극적 추출 예시:

문서에 "2024년 3월 15일부터 2024년 12월 31일까지" 라고 있으면:
- execution_period: "2024-03-15~2024-12-31"
- contract_duration: "2024-03-15~2024-12-31"

문서에 "계약금액: 50,000,000원(부가세 포함)" 이라고 있으면:
- contract_amount: 50000000
- contract_terms: "부가세 포함"

문서 제목이 "2020학년도 주약초등학교 영어방과후 프로그램 운영 용역" 이면:
- contract_title: "2020학년도 주약초등학교 영어방과후 프로그램 운영 용역"
- service_description: "영어방과후 프로그램 운영"

문서에 "육군 과학화전투훈련단" 이라고 있으면:
- ordering_agency: "육군 과학화전투훈련단"

문서에 "총액계약" 또는 "단가계약" 이라고 있으면:
- contract_type: "총액계약" 또는 "단가계약"

문서에 "월 1회 성과평가" 또는 "분기별 평가" 라고 있으면:
- performance_evaluation: "월 1회 성과평가" 또는 "분기별 평가"

문서에 "선금 30%, 중도금 40%, 잔금 30%" 라고 있으면:
- payment_method: "선금 30%, 중도금 40%, 잔금 30%"
