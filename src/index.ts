...
1. **정확성 우선**: 문서에 명시된 정보를 우선적으로 추출하세요.
2. **합리적 추론**: 명시되지 않았지만 문맥상 확실한 정보는 추론하여 추출하세요.
3. **형식 통일**: 금액은 숫자만, 날짜는 YYYY-MM-DD 형식으로 통일하세요.
4. **빈 값 처리**: 전혀 관련 정보가 없을 때만 빈 문자열("")로 표시하세요.

📋 추출 필드 상세 가이드:

- **계약명**: 문서 제목, 헤더, 또는 사업명에서 추출
- **사업범위**: 용역의 범위, 규모, 대상 지역 등
- **금액**: 계약금액, 예산, 총액 (숫자만 추출, 쉼표 제거)
- **수행기간**: 용역 수행 기간 (시작일~종료일)
- **용역기관**: 계약을 수행하는 업체명 또는 담당부서
- **발주처**: 계약을 발주하는 기관명 (문서 상단 기관명 참고)
...
- 계약명: "2020학년도 주약초등학교 영어방과후 프로그램 운영 용역"
